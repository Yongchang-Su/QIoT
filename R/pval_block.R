...
pval_block <- function(Z, Y, block, k, c,method.list.all, Gam=1, opt.method = "Greedy",null.max = 10^5, stat.null = NULL, switch = FALSE){
  if(!is.factor(block)){
    block = as.factor(block)
  } 
  levels(block) = 1:length(levels(block))
  if(Gam>1){
    return(sen_ls_p(Z,Y,block,k,c,gam=Gam,method.list.all,opt.method, switch))
  }else if(is.null(stat.null)){
    stat.null = null_dist_block(Z, block, method_list_all = method.list.all, null_max = null.max)
  }
  stat = min_stat_block(Z, Y, block, k = k, c, method.list.all, opt.method = opt.method, ties = c("upper", "lower"))
  pval = list(upper = mean(stat.null+1e-10>= stat$lower), lower = mean(stat.null+1e-10>= stat$upper))
  return(pval)
}
