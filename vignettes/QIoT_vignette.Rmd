...
for(i in 1:7){
  gam = gammas[i]
  CIlist[[i]] = QIoT::ci_quantile_sen(Z, Y, block, gam=gam, method.list.all=method.list.all,switch = TRUE)
}
```

```{r}
plot(CIlist[[1]][CIlist[[1]]>-Inf], (1:length(CIlist[[1]]))[CIlist[[1]]>-Inf], type = "p", xlim = c(-0.5, 1), xlab = "c", ylab = "k", cex = 0.5)
for(i in 2:7){
  points(CIlist[[i]][CIlist[[i]]>-Inf], (1:length(CIlist[[i]]))[CIlist[[i]]>-Inf], cex = 0.5, col = i) 
}
...
pval_quantile_sen(Z, Y, block, k, 0, gam=gam, method.list.all=method.list.all, switch = TRUE)$upper
...
for(i in 1:rep_time){
  Y = rnorm(S*n) + Z
  for(j in 1:10){
    if(QIoT::pval_quantile_sen(Z,Y,block,floor(0.95*N),0, gam=1+0.1*(j-1),method.list.all=method.list.all)$lower<0.1){
      no_sw[j] = no_sw[j] + 1
    }
    if(QIoT::pval_quantile_sen(Z,Y,block,floor(0.95*N),0, gam=1+0.1*(j-1),method.list.all=method.list.all,switch = TRUE)$lower<0.1){
      sw[j] = sw[j] + 1
    }
  }
}
